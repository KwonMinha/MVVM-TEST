...
class WeatherViewModelTests: XCTestCase {
  ...
  func testChangeLocationUpdatesLocationName() {
    ...
    /*
     3 - Bind to locationName and only fulfill the expectation if the value matches the expected result.
    ...
    /*
     4 - Begin the test by changing the location.
     It’s important to wait a few seconds before making the change so that any pending geocoding activity completes first. When the app launches, it triggers a geocoder lookup.
